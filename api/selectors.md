...
- `path` — строка. Точный path либо pattern с wildcard `*` (один
  сегмент) и `**` (любая глубина). Поддерживаются формы: exact
  (`a/b/c`), `prefix/**` (всё под prefix), `prefix/*` (прямые дети
  prefix-а) и bare `**` (любой path в scope). Wildcard в середине
  pattern-а (`a/*/b`) и одиночный `*` без `/` — `invalid_request`
  с `reason="unsupported_path_pattern"`.
...
Все main-узлы под `DocsWalker/`, привязанные к ветке `документы/спека`
карты `категория`.
...
Возвращает узлы, у которых есть исходящий link `зависит-от` на узел,
...
В `links.from`/`links.to` допустима также alias-форма
`{ "alias": "<name>" }` (как в endpoint tx-операций) — подставляет
набор узлов, объявленный в более раннем `select.as` этой же `read`.
Незнакомый alias — `unknown_alias`. См. раздел «Aliases» ниже.
