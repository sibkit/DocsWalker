...
для `read`; `select` / `create` / `update` / `move` / `delete` / `link`
...
`selector.match.fields` содержит значение за пределами набора (`title`,
`value`).

### `match_timeout`

Regex-фильтр `selector.match` превысил bounded timeout. Подсказка:
сузить `path` / `map_bindings` либо упростить regex.
...
`defaults.path_parent` задан, и `path` внутри `create`, `move.to` или
selector slot — абсолютный.
...
`rollback.tx_id` отсутствует в hist.
...
компактными summary блокирующих tx.
...
Изменения готовы к применению, запись соответствующих hist-узлов
неуспешна. Tx считается не применённой; состояние main / usage / scheme
остаётся прежним.
