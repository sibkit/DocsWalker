...
Параметр `at` (см. [model.md](model.md), раздел «Темпоральные чтения
(`at`)») передан в контексте, темпоральные чтения не поддерживающем.
`details.reason`:

- `hist_scope` — `read scope=hist` с `at`. Hist — append-only journal,
  хронологическая фильтрация делается через селектор `date` или `id`.
- `tx_method` — `tx` с `at`. Write над прошлым моментом не
  поддерживается.
- `meta_select` — `select: "meta"` с `at`. Meta-schema kernel-owned, в
  hist не фиксируется и версионируется с релизом kernel.
...
`update.expected_version` не совпадает с текущей ревизией узла —
узел изменился между чтением (откуда LLM взяла `version`) и записью.
...
LLM перечитывает узел через `read`, переосмысляет правку на новой
ревизии и отправляет новую `tx`.

## Schema

### `validation_failed`

Tx прошла resolve и concurrency-check, финальная validation графа
отклонила результат. `details.errors[]` — массив объектов
`{code, id, ...}` с подробностями каждого нарушения. Допустимые `code`
внутри `errors[]`:

- `unknown_map` — `{ code, id, map, branch? }`. `map` отсутствует в
  схеме target scope, либо `branch` отсутствует в этой map.
- `unknown_link` — `{ code, link, from, to }`. Имя link отсутствует в
  схеме target scope.
- `map_required_missing` — `{ code, id, map }`. У узла нет привязки к
  map, объявленной `required=true` в схеме target scope.
