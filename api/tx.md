...
Если фактический diff после `set` пустой (значения совпадают с
текущими), op считается no-op: `version` не инкрементируется, узел в
`changed.nodes` не попадает. `expected_version` всё равно сверяется
— это защита от гонки, а не от записи идентичного значения.
...
Узел, для которого фактического изменения нет (уже под нужным
`parent_path` и с запрошенными `map_bindings`), в `changed.nodes` не
попадает и `version` не инкрементируется. Если `to.map_bindings = {}`
и `to.parent_path` отсутствует — `move` целиком no-op для каждого узла
из селектора, ничего в hist не пишется. `expected_count` при этом
по-прежнему сверяется с размером selector.
