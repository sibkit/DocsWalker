...
- `title` (`compact`) — последний сегмент `path`. Соответствует regex
  ...
  underscore). Уникальность `path` проверяется по lower-case целиком:
  `A/B/c` и `a/b/c` считаются одним и тем же адресом и не могут
  сосуществовать. Оригинальный регистр символов сохраняется в самом
  узле и в ответе `read`.
...
- `description` (`compact`) — опциональный текст с подробностями
  транзакции. Без жёсткого лимита токенов, но входит в compact-форму
  для триажа hist без `include` (см.
  [hist-scope.md](hist-scope.md), раздел «Compact-форма event-узла»).
...
- `defaults.path_parent` — если задан, `path` внутри `create.path`,
  `move.to.parent_path`, `move.selector.path`, `delete.selector.path`,
  `select.selector.path` и в любом вложенном `selector.path` (например,
  в `selector.links.from`/`to`) интерпретируется как относительный
  внутри этого parent. Абсолютный `path` вместе с
