...
| target scope | state precondition | usage rule | usage map | usage link | project content gate |
|--------------|--------------------|-----------|-----------|------------|---------------------|
...
актуальный state или content `read_id` этого узла. Применимость:
...
State `read_id` принимается любым из state или content scope (content
...
scope (через `applies_to`-селектор в content rule, см.
[usage-scope.md](usage-scope.md)). Для каждого применимого rule LLM
передаёт content `read_id` этого rule-узла в `tx.read_ids`.
...
каждой назначаемой map LLM передаёт content `read_id` соответствующего
`usage/map`-узла (`map_bindings.category=usage/map` и
...
LLM передаёт content `read_id` соответствующего `usage/link`-узла
(`map_bindings.category=usage/link`, `link_name=<name>`).

## Project content gate (tx scope=main, по rule)

Если применимый `usage/rule` содержит в content
`requires_project_content_read=true`, для каждого main-узла, одновременно
затронутого tx и попадающего под `applies_to` этого rule, LLM передаёт
content `read_id` этого main-узла. Чтобы получить content `read_id`, LLM
делает full read через `read` с `include=["content"]` и без truncation.

Если rule не содержит `requires_project_content_read=true`, узел требует
...
- `reason` ∈ {`state_precondition`, `project_content`, `usage_rule`,
  `usage_map`, `usage_link`}.
- `read_scope` ∈ {`state`, `content`}.
...
- `scope_mismatch` — передан state `read_id` там, где требуется content;
...
1. `read` для нужного main-узла:
   ...
   ```
   получает узел `"2a"` с content `read_id`.
2. `read scope=usage` для применимого rule:
   ...
   ```
   получает rule с content `read_id`.
3. `read scope=usage` для map / link, назначаемых или создаваемых tx;
   получает content `read_id`-ы.
4. `tx`:
   ```json
   {
     "title": "...",
     "read_ids": ["4c8f", "31bf", "7bc0"],
     "ops": [ { "update": { "id": "2a", "set": { "content": "..." } } } ]
   }
   ```
