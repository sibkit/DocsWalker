...
`max_tokens` ограничивает размер выдачи в токенах, оценивая стоимость
**фактически кладущихся** полей. Compact-форма (без `include`) тратит
бюджет на id/path/title/map_bindings/scope?/tokens/version (≈20–60
токенов на data-узел, ≈30–50 на event-узел). `include=["content"]`
добавляет цену content, `include=["links"]` — incident links,
`include=["created"/"changed"/"deleted"]` для hist — стоимость
sections-json. Узел `tokens`-поле остаётся оценкой full-формы (для
информирования LLM), но budget на truncation считается по компактной
форме плюс реально включённые loadable.
