...
`value`, `map_bindings`. Идентификация — только по `id`. Иерархии под
hist нет.
  ...
  "nodes": [
    {
      ...
      "title": "selectors",
      "value": "...",
      "map_bindings": { "content": "documents/spec" }
    }
  ],
  "links": [
    { "name": "depends_on", "from": "c8", "to": "11" }
...
- `set` — те же ключи, что и в `tx.update.set` / `tx.move.to`
  (поля `title`, `value`, `path`, `map_bindings`). Заполнены **только
  те, что фактически поменялись** (forward-only diff, без prior
  значений).
  - Для скалярных полей (`title`, `value`, `path`) replay применяет
...
Селекторы `path`, `value`, `map_bindings`, `links` в hist неприменимы —
...
2. По хронологическому порядку `id` (или по `date` + порядку записи
   внутри даты) применяются все `hist/transaction`-узлы, относящиеся к
...
3. Внутри одной транзакции изменения применяются в фиксированном
   ...
   3. `changed.nodes` — применить `set` к каждому узлу: для скалярных
      полей (`title`, `value`, `path`) — полная замена поля; для
...
### Скалярные поля (`title`, `value`, `path`)
...
`value`, `path`) — алгоритмом для скалярных полей; `map_bindings` —
