...
- `set` — те же ключи, что и в `tx.update.set` / `tx.move.to`
  ...
  - Если `set` после фильтрации diff'а целиком пуст (op не изменила ни
    одного поля), узел в `changed.nodes` не попадает и `version` не
    инкрементируется. Tx остаётся успешной.
