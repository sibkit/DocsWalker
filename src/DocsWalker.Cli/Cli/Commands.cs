    ...
    private static List<CommandSpec> Build()
    {
        return new List<CommandSpec>
        {
            ...
            Read("find",
                ...
                examples: new[]
                {
                    "docswalker find --in-tree='[{\"name\":\"subject\",\"under\":42}]'",
                    "docswalker find --in-tree='[{\"name\":\"subject\",\"under\":42},{\"name\":\"subsystem\",\"under\":50}]' --type=rule --limit=20",
                    "docswalker find --in-tree='[{\"name\":\"subject\",\"under\":42}]' --compact=true",
                },
                Req("in_tree", ParamType.JsonArray, "Массив объектов {name, under}: имя classifier-дерева и id корня подграфа категорий, в котором ищем."),
                Opt("type",    ParamType.String,    "Фильтр по типу узла (TypeName из Схемы)."),
                Opt("limit",   ParamType.Integer,   "Максимум узлов в ответе; default 50."),
                Opt("compact", ParamType.Boolean,   "true → оставить только id,type,title (без text и out_refs); default false.")),
