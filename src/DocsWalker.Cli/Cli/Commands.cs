...
internal static class Commands
{
    ...
    private static List<CommandSpec> Build()
    {
        return new List<CommandSpec>
        {
            ...
            Read("get_nodes",
                desc: "Полные узлы по списку id. Возвращает 5 полей каждого узла: id, type, title, text, out_refs. Все узлы — полные; для дешёвого обзора используй get-subtree с --fields=title и --depth.",
