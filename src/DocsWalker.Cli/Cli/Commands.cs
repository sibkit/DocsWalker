...
internal static class Commands
{
    ...
    private static List<CommandSpec> Build()
    {
        return new List<CommandSpec>
        {
            ...
            Read("get_nodes",
                desc: "Полные узлы по списку id. Возвращает 5 полей каждого узла: id, type, title, text, out_refs. Прямые id никогда не фильтруются по seen-set; параметр --no-seen=true отключает фильтрацию для auto-include-целей (см. (#350) docs/DocsWalker.yml).",
                examples: new[] { "docswalker get-nodes --ids=1,8,42", "docswalker get-nodes --ids=42 --no-seen=true" },
                Req("ids",     ParamType.IdList, "Один id или список id через запятую."),
                Opt("no-seen", ParamType.String, "true/false. Отключает seen-фильтрацию auto-include-целей (контекст-aware-выдача); seen всё равно обновляется. По умолчанию false.")),
            Read("get_by_path",
