...
## Шаги (черновик; разрез обсуждаем перед stg-0010 step-01)
...
## Замечание по разрезу на страты

Блок 3 (addressable trees) технически независим от блоков 1+2 (он про
API + meta-schema, не про процесс). Можно вынести в отдельную страт
`stg-0011.addressable-trees`. Решение принимаем перед step-01: если
diff в `docs/` от блоков 1+2 уже большой — режем, чтобы сохранить
атомарность шагов; если diff умеренный — оставляем в одной страт.
...
3. Перед началом step-01 — обсудить с пользователем разрез: одна страт
   или две (см. «Замечание по разрезу»).
4. Прочитать первый `[*]`-step и работать строго по нему. Step-файлы
   создаются перед началом каждого шага.
5. После завершения каждого шага — `[*] → [+]`, atomic git
   add / commit / push.
...
номер 9. JSON-storage переезжает на следующий свободный — вероятно
`stg-0011.storage-format-json` (если addressable-trees не расщепляется)
или `stg-0012.storage-format-json` (если расщепляется). Окончательная
нумерация JSON-storage — по факту перед его стартом.
