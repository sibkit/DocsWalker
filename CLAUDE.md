...
## Исключение: JSON API — в `api/`

Спецификация LLM-facing JSON API DocsWalker (методы, форма запросов и
ответов, операции записи, ошибки, workflow, hit/tx/query/scheme) живёт
**только** в папке [api/](api/) в формате markdown. Это единственный
source of truth для JSON API.

- Читаем и правим JSON API только в `api/**/*.md`. Обычные `Read`, `Edit`,
  `Write`, `Grep`, `Glob` — разрешены (это markdown, не граф DocsWalker).
- Файл `docs/DocsWalker-LLM JSON API.yml` **устарел**. Не читать, не
  править, не использовать как ориентир. Он будет удалён отдельным шагом.
- Если правишь поведение, формат, операцию или код ошибки JSON API —
  сначала обновляешь `api/`, затем код и тесты.
