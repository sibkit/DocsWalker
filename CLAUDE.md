...
**stg-0010.kernel-as-service** — 6 шагов; step-01, step-02, step-03,
step-04 в `[+]`; step-05, step-06 в `[ ]`.

**Где остановился:** step-04 (kill-auto-spawn) **завершён** (commit
`a684cbe`). Зачищен весь dead code, оставшийся unreferenced после step-03:

- **Удалены целиком (CLI):** `src/DocsWalker.Cli/Cli/Kernel/KernelClient.cs`,
  `KernelSpawner.cs`, `KernelLock.cs`, `KernelDiscovery.cs`,
  `KernelInfoFile.cs`. После этого папка `src/DocsWalker.Cli/Cli/Kernel/`
  содержит только три актуальных файла: `ClientConfig.cs`,
  `KernelHttpClient.cs`, `KernelJsonContext.cs`.
- **Удалён (Core):** `src/DocsWalker.Core/Server/StalePidDetector.cs`.
  Папка `Server/` после этого пуста (git её не отслеживает).
- **Частичная зачистка `KernelJsonContext.cs` (CLI):** удалён `record
  KernelInfo(...)` + `[JsonSerializable(typeof(KernelInfo))]` +
  doc-комментарий о discovery-файле. Остались `HealthResponse`,
  `GraphInfo`, `GraphsResponse` и их JsonSerializable.
- **`kernel/Program.cs`:** убран `using DocsWalker.Core.Server;` —
  unused после удаления StalePidDetector. `kernel_already_running`
  проверка уже была убрана step-03 (проверено Grep'ом — отсутствует
  и в src/, и в tests/).

`dotnet build` + `dotnet test` **178/178** зелёные после step-04
(0 warnings, 0 errors).

**Что следующее (step-05 error-case-tests):** добавить тесты на новые
error-codes из step-03:
- `client_config_not_found`, `client_config_invalid`,
  `kernel_unreachable` — частично покрыты в `ClientConfigTests.cs`,
  но e2e-кейс «kernel offline → клиент даёт `kernel_unreachable`»
  через `KernelHttpClient.SendCommandAsync` пока не тестируется.
- Edge-cases для addressable-trees (step-02): `move-node` между
  parent'ами с collision title, `update-node` title с collision,
  `transaction` с collision внутри батча. Если что-то из этого уже
  покрыто в `AddressableTreeTests.cs` — пропустить; иначе добавить.

После step-05 — step-06 (smoke) на published binaries: ручной запуск
kernel-config с двумя graphs, `.dw/client.json` в двух разных проектных
папках, проверка всех новых error-codes и `--root=` → `unknown_parameter`.
