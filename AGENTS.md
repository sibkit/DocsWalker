...
Инструкции для Codex в этом репозитории. Спецификация JSON API живет в
`api/`; остальная продуктовая спецификация живет в `docs/`. Этот файл
описывает рабочий протокол агента и локальную интеграцию инструментов.
...
- `api/` - single source of truth для описания JSON API.
- `docs/` - single source of truth для всей остальной продуктовой спецификации.
  Не фиксируй продуктовые решения в `AGENTS.md`, `CLAUDE.md` или временных
  заметках.
- Для любой продуктовой фичи сначала уточняется спецификация: JSON API - в
  `api/`, остальное - в `docs/`, затем меняется код. Если реализация изменила
  поведение, синхронно обновляй соответствующий source of truth.
- Если инструкции, `api/` или docs противоречат друг другу вне явно заданного
  разделения ответственности, остановись и явно сообщи пользователю. Для JSON
  API приоритет имеет `api/`; для всего остального - `docs/`. Не выбирай
  удобную трактовку молча.

## Текущее ограничение

- В текущем потоке Codex работает только с папкой `api/`: читать, искать и
  редактировать нужно только файлы внутри `api/`.
- Не менять `src/`, `tests/`, `docs/`, `api_dw/` и другие директории без нового
  прямого запроса пользователя.
- Исключение - этот `AGENTS.md`, потому что пользователь прямо попросил
  зафиксировать ограничение.

## Работа с `api/`

`api/` содержит человекочитаемое описание JSON API и является источником
правды для API-контрактов, форматов запросов и ответов, операций, ошибок и
LLM workflow. Эти файлы можно читать, искать и редактировать напрямую.

Не дублируй API-решения в `AGENTS.md`, `CLAUDE.md` или временных заметках.
Если меняется поведение JSON API, сначала обнови релевантный файл в `api/`,
затем код и тесты.

Перед изменением API-контракта обязательно запроси подтверждение пользователя:
это касается добавления, удаления или переименования методов, операций, полей
команд, кодов ошибок, а также изменения JSON-структуры запросов и ответов.
...
3. Перечитать спецификацию JSON API в `api/`.
...
truth для JSON API остается в `api/`, source of truth для остального поведения
остается в `docs/`, а технический план и handoff - в
`.planning/stg-0012.../strategy.md`.
